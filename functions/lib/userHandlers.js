 ...
 */
async function createUserHandler(data, ctx, svc) {
    const callerUid = (0, validation_1.requireAuth)(ctx);
    await (0, validation_1.requireActiveAdmin)(svc, callerUid);
    const email = (0, validation_1.assertString)(data?.email, 'email');
    const password = (0, validation_1.assertString)(data?.password, 'password');
    const displayName = (0, validation_1.assertString)(data?.displayName, 'displayName');
    const role = (0, validation_1.assertString)(data?.role, 'role');
    (0, validation_1.assertValidEmail)(email);
    (0, validation_1.assertValidPassword)(password);
    (0, validation_1.assertValidCreateRole)(role);
    let uid;
    try {
 ...
 */
async function deleteUserHandler(data, ctx, svc) {
    const callerUid = (0, validation_1.requireAuth)(ctx);
    await (0, validation_1.requireActiveAdmin)(svc, callerUid);
    const uid = (0, validation_1.assertString)(data?.uid, 'uid');
    (0, validation_1.assertNonEmptyId)(uid, 'uid');
    if (uid === callerUid) {
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', 'User data was removed, but the authentication account could not be deleted. Please contact support.');
    }
    return { success: true, uid, message: 'User deleted successfully.' };
}
function mapCreateAuthError(err, email) {
    const code = getErrorCode(err);
    if (code === 'auth/email-already-exists' || code === 'EMAIL_EXISTS') {
        return new https_1.HttpsError('already-exists', `A user with the email ${email} already exists.`);
    }
    if (code === 'auth/invalid-email' || code === 'INVALID_EMAIL') {
        return new https_1.HttpsError('invalid-argument', 'The provided email address is invalid.');
    }
    if (code === 'auth/invalid-password' || code === 'INVALID_PASSWORD') {
        return new https_1.HttpsError('invalid-argument', 'The provided password is invalid.');
    }
    return new https_1.HttpsError('internal', 'Failed to create the user account. Please try again.');
}
function mapProfileError(err) {
    const code = getErrorCode(err);
    return new https_1.HttpsError('internal', code === 'unavailable'
